...
scene.onOverlapTile(SpriteKind.Player, sprites.dungeon.stairNorth, function (sprite, location) {
    ...
    tiles.placeOnTile(sprite, tiles.getTileLocation(8, 1))
...
function Respawn () {
    ...
    if (RoomNumber == 5) {
        tiles.placeOnTile(Character, tiles.getTileLocation(8, 1))
    }
    statusbar.value = 100
...
function ExitSideRooms () {
    ...
    if (chests_opened >= 3) {
        tiles.setTileAt(tiles.getTileLocation(0, 0), assets.tile`myTile12`)
    }
    RoomNumber = 1
}
